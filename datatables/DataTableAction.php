...
class DataTableAction extends Action
{
    ...
    /**
     * @see \coreb2c\smartadmin\DataTableAction::getParam
     * @var string
     */
    ...
    public function applyFilter(ActiveQuery $query, $columns, $search)
    {
        ...
        foreach ($columns as $column) {
            if ($column['searchable'] == 'true' && array_key_exists($column['name'], $schema) !== false) {
                $value = empty($search['value']) ? $column['search']['value'] : $search['value'];
                $query->orFilterWhere(['like', $column['name'], $value]);
            }
        }
        return $query;
    }
    ...
    public function applyOrder(ActiveQuery $query, $columns, $order)
    {
        ...
        foreach ($order as $key => $item) {
            ...
            $query->addOrderBy([$columns[$item['column']]['name'] => $sort]);
        }
        return $query;
    }
